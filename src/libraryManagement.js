...
export const projectLibrary = (() => {
  ...
  if (window.localStorage
    ...
    arr = JSON.parse(window.localStorage
      .getItem('project-library'))
      .map(project => {
        return new Project(project.title, project.description, project.dueDate, project.notes, project.tasks, project.identifier, project.complete)
      })
  }
  const show = () => arr
  const updateLocalStorage = () => {
...
export const addNewProject = () => {
  ...
  const tasks = Array.from(document.getElementsByClassName('task-list-item')).filter(item => {
    ...
    temp.forEach(item => {
      if (tempArr.length === 0) {
        ...
        for (let i = tempArr.length - 1; i >= 0; i--) {
          const test = compareAsc(parseISO(item.dueDate), parseISO(tempArr[i].dueDate))
          if (test === 1 || test === 0) {
